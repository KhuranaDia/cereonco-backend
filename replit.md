...
- **Google sign-in via Auth0 access token**: `POST /auth/google` accepts `{ accessToken }` (an Auth0 access token the frontend obtained after Auth0 login — Auth0 can broker Google). The server verifies it against the Auth0 `/userinfo` endpoint (`utils/auth0.ts` → `verifyAuth0AccessToken`) and trusts ONLY the profile Auth0 returns — never a raw client-supplied profile. It then reuses the lookup-by-email-then-`googleSub`, create-if-missing (`role=patient`, `emailVerified=true`, placeholder email `${sanitizedSub}@google.local`), backfill-`googleSub`/avatar logic and returns the same `{ token, user }` payload as login. Requires the `AUTH0_DOMAIN` env var (non-secret config; e.g. `your-tenant.auth0.com`). Errors: missing `accessToken` → 400; invalid/expired token → 401; `AUTH0_DOMAIN` unset → 503.
